...
void about()
{
    cout << "******************************************************************" << endl;
    cout << "\t"
         << "SJF PREEMPTIVE CPU SCHEDULING ALGORITHM SIMULATION" << endl;
    cout << "******************************************************************" << endl;
}

void PrintProcessData(Process process)
{
    cout << process.id << "\t" << process.arrival_time << "\t" << process.burst_time << "\t" << process.finish_time << "\t" << process.wait_time << endl;
}
...
int main()
{
    ...
    while (1)
    {
        ...
        int temp_burst_time = INT_MAX;
        for (int i = 0; i < pcount; i++)
        {   
            if (
                processes[i].rem_burst_time && 
                (processes[i].burst_time < temp_burst_time) && 
                (processes[i].arrival_time <= current_time)
            ) {
                short_burst_index = i;
                temp_burst_time = processes[i].burst_time;
            }
        }
