...
{
    cout << "************************************************************************" << endl;
    cout << "\t" << "PRIORITY NON PREEMPTIVE CPU SCHEDULING ALGORITHM SIMULATION" << endl;
    cout << "************************************************************************" << endl;
}

void PrintProcessData(Process process)
{
    cout << process.id << "\t" << process.priority << "\t" << process.arrival_time << "\t" << process.start_time << "\t" 
...
int main()
{
    ...
    for (int i = 1; i < pcount; i++)
    {
        ...
        for (int j = i; j < pcount; j++)
        {
            ...
            {
                if (processes[j].priority < processes[best_burst_index].priority)
                {
                    best_burst_index = j;
                }
            }
